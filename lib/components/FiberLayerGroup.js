...
export default class FiberLayerGroup extends Component {
  ...
  render(){
    ...
    const geometryElements = _.map(fibers, (fiber,i) => {
      ...
      var color = 'green'
      if(fiber.isSelected) {
          //console.log(fiber.center.latitude)
    ...
    /*const pathElements = _.map(fibers, (fiber,i) => {
      if(fiber.isSelected) {
        return <PathGeometry center={fiber.center} selectedPosition={selectedPosition} key={i} map={map}/>
      }
    })*/
